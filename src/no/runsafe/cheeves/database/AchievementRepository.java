...
public class AchievementRepository extends Repository
{
	...
	{
		HashMap<IPlayer, List<Integer>> achievements = new HashMap<>();
		ISet data = this.database.query("SELECT player, achievementID FROM cheeves_data");
		for (IRow node : data)
		{
			IPlayer player = node.Player("player");
			if (player == null)
				continue;

			if (!achievements.containsKey(player))
				achievements.put(player, new ArrayList<>());
	...
	{
		this.database.execute("UPDATE cheeves_data SET toasted = 1 WHERE player = ? AND toasted = 0", player);
	}
